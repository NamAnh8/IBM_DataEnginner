...
mysql --host=127.0.0.1 --port=3306 --user=root --password=MjA0MTgtbmFtYW5o -e "create database sales;
...
mysql --host=127.0.0.1 --port=3306 --user=root --password=MjA0MTgtbmFtYW5o -e "use sales;load data infile '/home/project/sales_olddata.csv' into table sales_data fields terminated BY ','lines terminated BY '\n';"

# Loading data from sales_newdata.csv into sales_data table.This csv consists of data updated with the current timestamp.
mysql --host=127.0.0.1 --port=3306 --user=root --password=MjA0MTgtbmFtYW5o -e "use sales;load data infile '/home/project/sales_newdata.csv' into table sales_data fields terminated BY ',' lines terminated BY '\n'(rowid,product_id,customer_id,price, quantity);"
